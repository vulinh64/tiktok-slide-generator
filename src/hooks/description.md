...
- **Notes:** Auto-calls `refresh()` on mount. `saveDeck` takes title, pages, optional existing id, optional `customCss`, optional `canvasSize`, optional `codeFont` (`'jetbrains' | 'consolas'`), and optional deck-level `fontFamily`; returns the saved deck id and refreshes the list. Also exports `SerializedPage`, `SlideDeck`, `SlideDeckFull`, and `CodeFont` types - `SlideDeck` is re-imported as a type by `Home.tsx` and `SlideManager.tsx`; `CodeFont` is consumed by `App.tsx`. Consumed by `App.tsx`.
...
- **Notes:** Takes the Tiptap editor as its only argument and listens to its `update` event to write the current page HTML back into the array. Uses refs (`pagesRef`, `activePageRef`, `metaRef`, `dirtyRef`) so callbacks never read stale state, and a `suppressSave` flag to avoid feedback loops when `setContent` is called during page switching/loading/export. Tracks per-page dirty state for partial-save optimizations. New pages inherit the active page's font and margin scale, while other page metadata starts from defaults. Page metadata is normalized to scale, margin, dark mode, and custom CSS only; font choices are deck-level. `replaceUrlInPages` rewrites image URLs across every page (used after image upload renames). Consumed by `App.tsx`.
