...
- **Notes:** Wires `useSlideEditor`, `usePages`, `useSlides`, `useSessionState`; handles `handleExport` / `handleExportAll` (single PNG and ZIP of PNGs); `handleExportAll` renders pages sequentially as `0000.png`, `0001.png`, etc. with a shared ZIP timestamp; `handleExportRaw` flushes the deck to disk and downloads `/api/slides/:id/export` (raw deck zip); `handleHomeImport` POSTs a user-picked zip to `/api/slides/import`, refreshes the list, and auto-opens the new deck. Owns deck-level state (title, deck CSS, canvas size, global font family, code font) and per-page meta; applies per-page meta plus the deck-level font to the live canvas during bulk export; persists deck on `beforeunload` via `navigator.sendBeacon`. Code-font choice is propagated as a `--code-font` CSS variable on `#slide-canvas`; default `jetbrains` leaves the variable unset so the CSS fallback in `components/Editor.css` applies. The sticky header puts back/title plus File/Deck/Page/View/Font/Images menus on the primary row (wraps as needed). The `skipNonExportedNodes` filter strips UI-only nodes (ProseMirror separator image, code-block language `<select>`) from PNGs.

### App.css
- **Purpose:** Layout/shell styles for App (sticky header menus, page list rail, canvas container, export overlay, modals).
